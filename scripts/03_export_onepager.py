...
def _build_highlights(summary: dict) -> list[str]:
    highlights = []
    highlights.append(f"Latest reporting month: {summary.get('latest_month', 'N/A')}")
    highlights.append(
        f"12-month uploads: {_format_number(summary.get('total_last12'))} papers"
    )
    prev = summary.get("total_prev12")
    if prev:
        highlights.append(f"Prior 12-month uploads: {_format_number(prev)} papers")
    yoy_pct = summary.get("total_yoy_pct")
    yoy_change = summary.get("total_yoy_change")
    if yoy_pct is not None:
        highlights.append(
            f"YoY growth: {yoy_pct:+.2f}% ({_format_number(yoy_change, '{:+,.0f}')} papers)"
        )
    highlights.append(
        f"Rolling averages: 6M {_format_number(summary.get('rolling6_avg'), '{:,.1f}')} / "
        f"12M {_format_number(summary.get('rolling12_avg'), '{:,.1f}')}"
    )
    highlights.append(
        f"Trend slope (last 36M): {summary.get('trend_slope', 0):+.1f} papers/month "
        f"({summary.get('trend_slope_pct', 0):+.2f}%)"
    )
    cagr = summary.get("cagr_5yr")
    if cagr is not None:
        highlights.append(f"5-year CAGR: {cagr * 100:+.2f}%")
    ...
    if seasonality:
        ...
        if strongest:
            highlights.append(f"Peak seasonal month: {month_names[int(strongest) - 1]}")
        if weakest:
            highlights.append(f"Low seasonal month: {month_names[int(weakest) - 1]}")
        highlights.append(f"Seasonality strength: {strength:.2f}%")

    return highlights[:8]


def _format_section(summary: dict, key: str, limit: int = 5) -> list[str]:
...
def _draw_bullets(c: canvas.Canvas, bullets: list[str], start_x: float, start_y: float, font="Helvetica", size=10) -> float:
    y = start_y
    c.setFont(font, size)
...
def _main(args: argparse.Namespace) -> int:
    ...
    c = canvas.Canvas(str(assets.output_pdf), pagesize=A4)
    width, height = A4
    margin = 2.0 * cm
    ...
    bullet_start = height - margin - 1.2 * cm
    c.setFont("Helvetica", 11)
    bullet_y = _draw_bullets(c, highlights, margin, bullet_start, size=10)

    # Category sections
    col_width = (width - 2 * margin - 1 * cm) / 2
    section_y = bullet_y - 0.3 * cm
    left_x = margin
    ...
    image_top = min(section_y_left, section_y_right) - 0.6 * cm
    available_width = width - 2 * margin
    gap = 0.4 * cm

    rows = [
        ([assets.total_fig], 5.0 * cm),
        ([assets.growth_fig, assets.share_fig], 5.0 * cm),
        ([assets.heatmap_fig, assets.scatter_fig], 5.0 * cm),
        ([assets.volatility_fig], 4.8 * cm),
    ]

    y = image_top
    for fig_paths, row_height in rows:
        ...
        if len(fig_paths) == 1:
            ...
            half_width = (available_width - gap) / 2
            left_fig, right_fig = fig_paths
            if left_fig.exists():
                c.drawImage(
                    str(left_fig),
                    margin,
